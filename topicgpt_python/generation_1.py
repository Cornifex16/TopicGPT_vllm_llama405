...
def generate_topics(
    ...
    top_p,
    verbose,
    early_stop=200,  # Modify this parameter to control early stopping
):
    """
    ...
    for i, doc in enumerate(tqdm(docs)):
        ...
        try:
            ...
            print("Topicos extraidos, previo regex", topics)
            for t in topics:
...
def generate_topic_lvl1_resume(
    api, model, data, prompt_file, seed_file, out_file, topic_file, log_file, verbose
):
    """
    ...
    # --- LOGICA DE REANUDACIÓN (RESUME LOGIC) ---
    start_index = 0
    
    # 1. Verificar si hay progreso guardado en el archivo de salida
    if os.path.exists(out_file) and os.path.getsize(out_file) > 0:
        try:
            existing_df = pd.read_json(out_file, lines=True)
            start_index = len(existing_df)
            print(f"--> Archivo de salida encontrado. Reanudando desde la línea {start_index}...")
        except ValueError:
            print("--> Archivo de salida corrupto o vacío. Empezando desde 0.")
    
    # Cortamos la lista de documentos para procesar solo los nuevos
    docs_to_process = docs[start_index:]
    
    if not docs_to_process:
        print("Todos los documentos ya han sido procesados.")
        return TopicTree.from_topic_list(topic_file, from_file=True) if os.path.exists(topic_file) else None

    # 2. Cargar el árbol de tópicos previo si existe (para no perder lo aprendido)
    generation_prompt = open(prompt_file, "r").read()
    
    if os.path.exists(topic_file) and os.path.getsize(topic_file) > 0:
        print(f"--> Cargando tópicos existentes desde {topic_file}")
        # Usamos from_topic_list porque el archivo generado tiene formato completo
        topics_root = TopicTree.from_topic_list(topic_file, from_file=True)
    else:
        print(f"--> Iniciando con tópicos semilla desde {seed_file}")
        topics_root = TopicTree().from_seed_file(seed_file)
        
    topics_list = topics_root.to_topic_list(desc=True, count=False)

    # Generate topics (pasamos solo la porción de docs que falta)
    responses, topics_list, topics_root = generate_topics(
        topics_root,
        topics_list,
        context_len,
        docs_to_process, # Usamos la lista cortada
        seed_file,
        api_client,
        generation_prompt,
        temperature,
    ...
    # Save generated topics (Siempre sobrescribimos el topic_file con la versión más actual)
    topics_root.to_file(topic_file)

    # --- GUARDADO INCREMENTAL ---
    try:
        # Creamos un DataFrame solo con los nuevos resultados
        # Aseguramos que los índices coincidan con la porción procesada
        new_df = df.iloc[start_index : start_index + len(responses)].copy()
        new_df["responses"] = responses
        
        # Si empezamos desde 0, modo 'w' (escribir/sobrescribir)
        # Si reanudamos, modo 'a' (append/agregar al final)
        write_mode = 'a' if start_index > 0 else 'w'
        
        new_df.to_json(out_file, lines=True, orient="records", mode=write_mode)
        print(f"--> Guardados {len(responses)} nuevos registros en {out_file}")
        
    except Exception as e:
        traceback.print_exc()
        # Backup simple en texto por si falla pandas
        with open(f"data/output/generation_1_backup_resume_{model.replace('/', '_')}.txt", "a") as f:
            for line in responses:
                print(line, file=f)
