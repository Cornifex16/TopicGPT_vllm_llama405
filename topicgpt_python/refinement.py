...
_sbert_model = SentenceTransformer("all-MiniLM-L6-v2", device=_device)

def ajustar_respuesta_unalinea(texto_rep: str):
    return regex.findall(r"\[\d+\][^\[]+", texto_rep)
 
def limpiar_markdown(texto: str):
    texto = texto.replace("**", "")
    texto = texto.replace("*", "")
    texto = texto.rstrip(",")
...
def topic_pairs(topic_sent, all_pairs, threshold=0.5, num_pair=2):
    ...
    """
    embeddings = _sbert_model.encode(topic_sent, convert_to_tensor=True)
    ...
    for pair in pairs:
        ...
        if (
            ...
            print("AVISO: pares de topic sent y puntaje:")
            print(topic_sent[i])
            print(topic_sent[j])
            print(pair["score"])
...
def merge_topics(
    ...
    new_pairs, all_pairs = topic_pairs(
        topic_sent, all_pairs=[], threshold=0.5, num_pair=10
    )
    ...
    while len(new_pairs) > 1:
        ...
        try:
            ...
            for linea in lineas:
                ...
                if not texto:
                    continue
                if texto.startswith("(") and merges:
                    merges[-1] = merges[-1] + " " + texto
                else:
                    merges.append(texto)


            for merge in merges:
                line = limpiar_markdown(merge.strip())
                lvl_match = regex.search(r"^\[(\d+)\]", line)
                if not lvl_match:
                    if verbose:
                        print("linea sin nivel, ignorando:", line)
                    continue
                ...
                for i in range(len(line) - 1, -1, -1):
                    if line[i] == "(":
                        candidate = line[i:]
                        m2 = regex.match(r"\(\s*(\[\d+\].+)\)\s*$", candidate, regex.DOTALL)
                        if m2 and regex.search(r"\[\d+\].*,.*\[\d+\]", m2.group(1)):
                            orig_start = i
                            orig_content = m2.group(1)
                            break
                if orig_start is None:
                    if verbose:
                        print("sin originales válidos, ignorando:", line)
                    continue
                original = orig_content
                main_part = line[:orig_start].strip()

                main_body = regex.sub(r"^\[\d+\]\s*", "", main_part).strip()
                if ":" not in main_body:
                    if verbose:
                        print("sin separador nombre:desc, ignorando:", main_body)
                    continue
                name, desc = main_body.split(":", 1)
                name = limpiar_markdown(name.strip())
                desc = limpiar_markdown(desc.strip())

                if not name or not desc:
                    if verbose:
                        print("nombre o descripción vacíos, ignorando:", line)
                    continue

                orig_matches = regex.findall(r"\[(\d+)\]\s*([^,\[\]()]+)", original)
                if len(orig_matches) < 2:
                    if verbose:
                        print("topicos originales invalidos:", original)
                    continue
                original_topics = [(t[1].strip(), int(t[0])) for t in orig_matches if t[1].strip()]
                if len(original_topics) < 2:
                    if verbose:
                        print("originales insuficientes tras filtrar:", original_topics)
                    continue
...
def update_generation_file(
    ...
    for i in range(len(responses)):
        if responses[i] != updated_responses[i]:
            print(f"RESPUESTAS DISTINTAS: ({responses[i]}) ==> ({updated_responses[i]})")
